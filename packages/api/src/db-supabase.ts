/**
 * Supabase-backed database adapter for Praetor API.
 *
 * This is the verbatim body of the original db.ts, extracted so the dispatcher
 * in db.ts can switch between this and db-memory.ts based on PRAETOR_DEV_MODE.
 *
 * In the current codebase `supabaseAdmin()` is actually the Praetor-native
 * in-memory store (see supabase.ts). If a real Supabase project is wired up
 * in the future, this file is the only one that needs to change.
 */
